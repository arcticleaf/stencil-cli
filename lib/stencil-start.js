...
class StencilStart {
    ...
    async run(cliOptions) {
        ...
        initialStencilConfig.channelIds = initialStencilConfig.channelIds?.length ? initialStencilConfig.channelIds : [1]
        ...
        const storeInfoFromAPI = await this._themeApiClient.checkCliVersion({
            storeUrl: channelUrl || initialStencilConfig.normalStoreUrl,
        });
        ...
        await this.startLocalServer(cliOptions, {
            ...updatedStencilConfig,
            port:
                cliOptions.disableBS !== true
                    ? updatedStencilConfig.port
                    : initialStencilConfig.port,
        });
        ...
        if (cliOptions.disableBS === true) {
            // don't run webpack, we don't need to since we're previewing
            if (cliOptions.displayShippyPath === true) {
                this._logger.log(
                    `{"shippy": {"url":"http://localhost:${initialStencilConfig.port}"}}`,
                );
            }
        } else {
